...
let showDetails = async (id) =>{
  ...
  detailsContainer.innerHTML = `
    ...
    <div class="p-5 bg-gray-300 mt-5">
        <p>WLAN: ${phoneData?.others?.WLAN || 'No Data Found'}</p>
        <p>Bluetooth: ${phoneData?.others?.Bluetooth || 'No Data Found'}</p>
        <p>GPS: ${phoneData?.others?.GPS || 'No Data Found'}</p>
        <p>NFC: ${phoneData?.others?.NFC || 'No Data Found'}</p>
        <p>Radio: ${phoneData?.others?.Radio || 'No Data Found'}</p>
        <p>USB: ${phoneData?.others?.USB || 'No Data Found'}</p>
    </div>
  `
}
